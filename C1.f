c******************************************************************************
c
c      Subroutines for compression stage
c
c******************************************************************************
c
       SUBROUTINE  DDTCVV(IERR)
c
c      Derivative calculations during compression stage
c-----------------------------------------------------------------------------
...
c          DVDT    dV/dt, m**3/s
c          FTDC    factor in turbulence dissipation during compression
c          FTPC    factor in turbulence production during compression
...
c          STC     Stanton number during compression
...
c    Variable locations in RW:
       ...
       EQUIVALENCE (STC ,RW(251))
       EQUIVALENCE (TW  ,RW(270))
       EQUIVALENCE (FTDC,RW(273))
       EQUIVALENCE (FTPC,RW(277))
c-----------------------------------------------------------------------------
c    heat transfer rate
       QDOT = STC*VELT*RHOC*CPC*AHTC*(TC - TW)
...
c    kinetic energy balance terms for semi-implicit advance
       DKTD = FTPC*ABS(VPIS*VPIS*VPIS)*AHTC/VC
       DCKT = 0.666667*DVDT/VC + FTDC*VELT/(VC**0.333333)
c    mass balances
       DMPD = 0
       DMRD = 0
c    exit
...
c******************************************************************************
c
       SUBROUTINE ADVCVV(DT,Y,YN,IERR)
c
c      Advances differential variables for compression stage from Y() to YN()
...
c----------------------------------------------------------------------------
       ...
       COMMON  /RWVV/  RW
c----------------------------------------------------------------------------
c    set derivative locations in RW
...
c******************************************************************************
c
       SUBROUTINE CPYCVV(Y,YN)
c
c      Copies differential variables for compression stage from Y to YN
c-----------------------------------------------------------------------------
...
c
       SUBROUTINE ALGCVV
c
c      Advances algebraic variables for compression stage
c-----------------------------------------------------------------------------
c    The following quantities must be loaded in RW at call:
c
